...
function () {
  ...
  createClass(Onion, [{
    ...
    value: function execute() {
      ...
      var fn = compose([].concat(toConsumableArray(this.middlewares), toConsumableArray(Onion.globalMiddlewares), toConsumableArray(Onion.coreMiddlewares)));
      return fn(params);
    }
  }]);
...
function timeout2Throw(msec, request) {
  return new Promise(function (_, reject) {
    setTimeout(function () {
      reject(new RequestError("timeout of ".concat(msec, "ms exceeded"), request));
    }, msec);
  });
...
function getParamObject(val) {
  if (isURLSearchParams(val)) {
    return queryString.parse(val.toString());
  }
...
function fetchMiddleware(ctx, next) {
  ...
  if (__umiRequestCoreType__ !== 'normal') {
    if (process && process.env && process.env.NODE_ENV === 'development') {
      console.warn('__umiRequestCoreType__ is a internal property that use in umi-request, change its value would affect the behavior of request! It only use when you want to extend the request core.');
    }
  ...
  var needCache = method.toLowerCase() === 'get' && useCache && isBrowser;
  ...
  if (params) {
    if (paramsSerializer) {
      ...
      if (isArray(params)) {
        ...
        serializedParams = queryString.stringify(jsonStringifiedParams);
      } else {
        jsonStringifiedParams = {};
        ...
        var tmp = queryString.stringify(jsonStringifiedParams);
...
var requestInterceptors = [addfix];
var responseInterceptors = []; // 初始化全局和内核中间件
